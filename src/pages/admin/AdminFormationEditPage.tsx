...
import { ArrowLeft, Save, LayoutGrid, Tag as TagIcon, Settings2, Image as ImageIcon, Loader2 } from "lucide-react";
...
const AdminFormationEditPage = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  // Fetch the formation to edit
  ...
  // Pre-fill form when formation data loads
  useEffect(() => {
    if (formation) {
      form.reset({
        ...
        formatId: formation.format?.id,
        formateurId: formation.formateur?.id,
        tagIds: formation.tags?.map(t => t.id) || [],
        competenceIds: formation.competences?.map(c => c.id) || [],
        cout: formation.cout,
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => formationsService.updateFormation(id!, values as any),
    onSuccess: () => {
      toast.success("Formation mise à jour avec succès !");
      queryClient.invalidateQueries({ queryKey: ['admin-formations'] });
      queryClient.invalidateQueries({ queryKey: ['admin-formation', id] });
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Ex: Bootcamp React Avancé 2024" className="h-11 rounded-xl bg-slate-50/50 border-slate-200 focus-visible:ring-primary/20 focus-visible:border-primary transition-all text-slate-800" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Maîtrisez les hooks, le state management et l'architecture" className="h-11 rounded-xl bg-slate-50/50 border-slate-200 focus-visible:ring-primary/20 focus-visible:border-primary transition-all text-slate-800" {...field} />
                      </FormControl>
                ...
                <FormField
                  ...
                  render={() => (
                    <FormItem>
                      ...
                      <div className="space-y-3 p-4 bg-slate-50/50 rounded-xl border border-slate-100 h-64 overflow-y-auto">
                        {tags?.map((item) => (
                          <FormField
                            key={item.id}
                            control={form.control}
                            ...
                            render={({ field }) => (
                              <FormItem key={item.id} className="flex flex-row items-center space-x-3 space-y-0 group">
                                ...
                                <FormLabel className="font-medium text-slate-600 group-hover:text-slate-900 cursor-pointer text-sm">
                                  {item.titre}
                                </FormLabel>
                              </FormItem>
                            )}
          ...
          <div className="space-y-8">
            ...
            {/* Image Card */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4 animate-fade-in-up">
              <h3 className="font-semibold text-slate-800 font-montserrat flex items-center gap-2">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-slate-700 text-sm">Image de couverture</FormLabel>
                    <FormControl>
                    ...
                    {field.value && (
                      <div className="mt-3 rounded-lg overflow-hidden border border-slate-200 aspect-video bg-slate-100">
                        <img src={field.value} alt="Aperçu" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                      </div>
                    )}
                  </FormItem>
                )}
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-5 animate-fade-in-up">
              <h3 className="font-semibold text-slate-800 font-montserrat">Tarification & Planification</h3>
              <FormField
                control={form.control}
                name="cout"
                render={({ field }) => (
...
export default AdminFormationEditPage;
